...
;	// Window title
	...
	Gui, 4:add, Text, xp+40 yp+5 backgroundtrans, % T.UI_CLOSE
...
Gui, 4:Add, Tab2 , +Theme -Background x20 y30 w500 h20, RegnumStart|Game|Graphics|Sound|Controls|Debug
	...
	;	// regnum path
	gui, 4:Tab, 1
gui  4:add, text, x15 y60 backgroundtrans, % T.REGNUM_PATH ":"
		Gui, 4:Font, s7
	...
	;	// hide NGE intro
		gui, 4:add, checkbox, w%CBW% h%CBH% x15 y+25 checked%skip_logo% backgroundtrans vskip_logo
		gui, 4:add, text, x+3 yp backgroundtrans, % T.DELETE_SPLASH

	;	// hide loading screen
	  gui, 4:add, checkbox, w%CBW% h%CBH% x15 y+8 checked%hide_loading_screen% backgroundtrans vhide_loading_screen
		gui, 4:add, text, x+3 yp backgroundtrans, % T.HIDE_LOADING_SCREEN

	;	// screenshot quality
		gui, 4:add, checkbox, w%CBW% h%CBH% x15 y+8 checked%screenshot_quality% backgroundtrans vscreenshot_quality
		gui, 4:add, text, x+3 yp backgroundtrans, % T.SCREENSHOT_QUALITY

	;	// screenshot autosave
		gui, 4:add, checkbox, w%CBW% h%CBH% x15 y+8 checked%screenshot_autosave% backgroundtrans vscreenshot_autosave
		gui, 4:add, text, x+3 yp backgroundtrans, % T.SCREENSHOT_AUTOSAVE

	;	// server time and weather
		gui, 4:add, checkbox, x15 y+8 checked%dbg_ignore_server_time% backgroundtrans w%CBW% h%CBH% vdbg_ignore_server_time
	...
	;	// run as windows user
		gui, 4:add, checkbox, x15 y+15 checked%runas% w%CBW% h%CBH% grunAsGuiToggled vrunas
		gui, 4:add, text, x+3 yp backgroundtrans, % T.RUN_AS

		Gui, 4:Font, s7 cD8D8D8, Verdana
		gui, 4:add, text, x15 y+8 backgroundtrans vgui_runas_name_text, % "Windows " T.USER ; windows user text
		gui, 4:add, text, x140 yp backgroundtrans vgui_runas_pw_text, % "Win " T.PASSWORD ; windows password text
		Gui, 4:Font, s8 c000000, Verdana

		gui, 4:add, edit, x15 y+5 w90 h18 -multi vrunas_name, %runas_name%
	...
	gui, 4:Tab,2
		...
		gui, 4:add, checkbox, w%CBW% h%CBH% x15 y60 checked%cl_update_all_resources% backgroundtrans vcl_update_all_resources
		gui, 4:add, text, x+3 yp backgroundtrans, % T.UPDATE_ALL_RES

		gui, 4:add, checkbox, x15 y+8 checked%ingame_log% backgroundtrans w%CBW% h%CBH% vingame_log
		gui, 4:add, text, x+3 yp backgroundtrans, % T.INGAME_LOG

	gui, 4:Tab, 3
; row1
	;	// fullscreen mode
		gui, 4:add, checkbox, x15 y60 checked%vg_fullscreen_mode% backgroundtrans w%CBW% h%CBH% vvg_fullscreen_mode
		gui, 4:add, text, x+3 yp backgroundtrans, % T.FULLSCREEN_MODE

	;	// vsync
		gui, 4:add, checkbox, x15 y+8 checked%vg_vertical_sync% backgroundtrans w%CBW% h%CBH% vvg_vertical_sync
	...
	gui, 4:Tab, 4
		gui  4:add, text, backgroundtrans x15 y80, coming soon
	gui, 4:Tab, 5
		gui  4:add, text, backgroundtrans x15 y80, coming soon
	gui, 4:Tab, 6
	;	// change 64bit mode
		gui, 4:add, checkbox, w%CBW% h%CBH% x15 y60 checked%win64% backgroundtrans vwin64
		gui, 4:add, text, x+3 yp backgroundtrans, % T.64BIT_MODE

		;	// hide window border
		gui, 4:add, checkbox, w%CBW% h%CBH% x15 y+8 checked%hide_window_border% backgroundtrans w%CBW% h%CBH% vhide_window_border
		gui, 4:add, text, x+3 yp backgroundtrans, % T.HIDE_WINDOW_BORDER

	;	// fake net lag
		gui, 4:add, text, x15 y+8 backgroundtrans, % T.NET_FAKE_LAG " (ms)"
		gui, 4:add, edit, x150 yp w60 h15 -multi vnet_fake_lag, %net_fake_lag%

	;	// debug mode
		gui, 4:add, checkbox, w%CBW% h%CBH% x15 y+8 checked%debug_mode% backgroundtrans vdebug_mode
		gui, 4:add, text, x+3 yp backgroundtrans, % "debug mode (experimental (?))"

	;	// cl_crafting_show_min_level
		gui, 4:add, checkbox, w%CBW% h%CBH% x15 y+8 checked%cl_crafting_show_min_level% backgroundtrans w%CBW% h%CBH% vcl_crafting_show_min_level
		gui, 4:add, text, x+3 yp backgroundtrans, % "cl_crafting_show_min_level (experimental)"

	;	// cl_show_subclass_on_players
		gui, 4:add, checkbox, w%CBW% h%CBH% x15 y+8 checked%cl_show_subclass_on_players% backgroundtrans w%CBW% h%CBH% vcl_show_subclass_on_players
		gui, 4:add, text, x+3 yp backgroundtrans, % "cl_show_subclass_on_players (experimental)"

	;	// cl_show_hidden_armors
		gui, 4:add, checkbox, w%CBW% h%CBH% x15 y+8 checked%cl_show_hidden_armors% backgroundtrans w%CBW% h%CBH% vcl_show_hidden_armors
